...
void init_logger_thread() {
	// Open log file 
	// Logger thread will have access to this file 
...
void* worker() {
	while (1) {
		...
		while (active_check) {
			...
			char buf[1024] = { 0 };
			if (read(client_file_desc, buf, 1024) == 0) active_check = false;
			if (active_check) {
